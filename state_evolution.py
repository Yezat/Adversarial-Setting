...
# @nb.njit
# def _N_hat_integrand(xi: float, y: float, m: float, q: float, ρ: float, tau: float, epsilon: float, P: float, N: float, sigma: float) -> float:
#     e = m * m / (ρ * q)
#     w_0 = np.sqrt(ρ*e) * xi
#     V_0 = ρ * (1-e)
#     z_0 = math.erfc((-y * w_0) / np.sqrt(2*(tau**2 + V_0)))
...
def logistic_N_hat_func(
    ...
    return 0.0
    # Iplus = quad(lambda xi: _N_hat_integrand(xi,1, overlaps.m, overlaps.q, ρ, tau, epsilon, overlaps.P, overlaps.N, overlaps.sigma),-int_lims,int_lims,limit=500)[0]
    # Iminus = quad(lambda xi: _N_hat_integrand(xi,-1, overlaps.m, overlaps.q, ρ, tau, epsilon, overlaps.P, overlaps.N, overlaps.sigma),-int_lims,int_lims,limit=500)[0]
    # return alpha * (Iplus + Iminus)
...
def ridge_q_hat_func(task, overlaps, data_model, logger):
    ...
    return (
        ...
        * (
            adv**2
            + overlaps.q
            - 2 * overlaps.m * adv * np.sqrt(2 / (np.pi * data_model.ρ))
        )
        / (1 + overlaps.sigma) ** 2
    )
...
def ridge_P_hat_func(task, overlaps, data_model, logger):
    ...
    result = (
        task.alpha
        * (-adv + overlaps.m * np.sqrt(2) / np.sqrt(np.pi * data_model.ρ))
        * 1
        / (1 + overlaps.sigma)
    )
...
def var_hat_func(task, overlaps, data_model, logger=None):
    if task.problem_type == ProblemType.Ridge:
        ...
        or task.problem_type == ProblemType.EquivalentLogistic
        or task.problem_type == ProblemType.PerturbedBoundaryLogistic
        or task.problem_type == ProblemType.PerturbedBoundaryCoefficientLogistic
    ):
        ...
        overlaps.N_hat = logistic_N_hat_func(
            overlaps,
            data_model.ρ,
            task.alpha,
            task.epsilon,
        ...
        raise Exception(
            f"var_hat_func - problem_type {task.problem_type} not implemented"
        )
    return overlaps
...
class LogisticObservables:
    ...
    def training_loss_logistic_with_regularization(
        task: Task, overlaps: OverlapSet, data_model: AbstractDataModel, int_lims: float
    ):
        return (
            LogisticObservables.training_loss(task, overlaps, data_model, int_lims)
...
def compute_data_model_attackability(
    data_model: AbstractDataModel, overlaps: OverlapSet
):
    L = (
        overlaps.sigma_hat * data_model.spec_Σ_x
        + overlaps.P_hat * data_model.spec_Σ_δ
...
def asymptotic_adversarial_generalization_error(
    data_model: AbstractDataModel, overlaps: OverlapSet, epsilon, tau
):
...
def adversarial_generalization_error_overlaps_teacher(
    overlaps: OverlapSet, task: Task, data_model: AbstractDataModel, epsilon: float
):
    ...
    if task.tau >= 1e-10:
        ...
        return 1 - I / (np.sqrt(2 * np.pi * data_model.ρ))
...
def fixed_point_finder(
    ...
    while err > overlaps.TOL_FPE or iter_nb < overlaps.MIN_ITER_FPE:
        ...
        new_m, new_q, new_sigma, new_A, new_N, new_P, new_F = var_func(
            task, overlaps, my_data_model, logger
        )

        err = overlaps.update_overlaps(
            new_m, new_q, new_sigma, new_A, new_N, new_P, new_F
        )
