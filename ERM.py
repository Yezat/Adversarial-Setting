...
def run_optimizer(
    task: Task, data_model: AbstractDataModel, data: DataSet, logger, df_sigma
):
...
def preprocessing(coef, X, y, lam, epsilon, problem_type: ProblemType):
    ...
    if problem_type == ProblemType.Ridge:
        ...
        problem_type == ProblemType.EquivalentLogistic
        or ProblemType.PerturbedBoundaryLogistic
        or ProblemType.PerturbedBoundaryCoefficientLogistic
    ):
        target = y
    else:
        raise Exception(
...
class RidgeProblem:
    ...
    def compute_loss(coef, X, y, l2_reg_strength, epsilon, covariance_prior, Σ_δ):
        ...
        loss = (y - activation + y * adv_strength).T @ (
            y - activation + y * adv_strength
        ) + l2_reg_strength * (coef @ covariance_prior @ coef)
    ...
    def compute_gradient(coef, X, y, l2_reg_strength, epsilon, covariance_prior, Σ_δ):
        ...
        grad += (
            -2 * activation * adv_strength - 2 * epsilon * coef.T @ activation * Delta
        )
...
class LogisticProblem:
    ...
    def loss_gradient(
        ...
        epsilon_gradient_per_sample, gradient_per_sample = (
...
class EquivalentLogisticProblem:
    ...
    def loss_gradient(
        ...
        data_gradient_per_sample, attack_gradient_per_sample = (
            EquivalentLogisticProblem.compute_gradient(margins, optimal_attack)
        )
        ...
        adv_grad_summand = np.outer(
            attack_gradient_per_sample, derivative_optimal_attack
        ).sum(axis=0)
    ...
    def compute_loss(margins, optimal_attack):
        return log1pexp(-margins) + log1pexp(
            optimal_attack
        )  # - margins*optimal_attack/4
...
class PerturbedBoundaryLogisticProblem:
    ...
    def loss_gradient(
        ...
        cross_term = (
            -(1 / 4) * np.sum(margins[~mask_positive]) * optimal_attack / nww**2
        )
        direct_cross_term = (
            -(1 / 8)
            * epsilon
            * np.sum(2 * margins[~mask_positive] - optimal_attack)
            / nww
        )
        ...
        loss = PerturbedBoundaryLogisticProblem.compute_loss(
        ...
        positive_gradient_per_sample, negative_gradient_per_sample = (
            PerturbedBoundaryLogisticProblem.compute_gradient(
                shifted_margins_positive, shifted_margins_negative
            )
        )
        ...
        positive_contribution = (
            positive_label_data_product.T @ positive_gradient_per_sample
        )
        # log the shape
        ...
        negative_contribution = (
            negative_label_data_product.T @ negative_gradient_per_sample
        )
        # log the shape
...
class PerturbedBoundaryCoefficientLogisticProblem:
    ...
    def loss_gradient(
        ...
        gradient_per_sample = (
            PerturbedBoundaryCoefficientLogisticProblem.compute_gradient(margins)
        )
        ...
        adv_grad_summand = np.outer(
            gradient_per_sample, -derivative_optimal_attack
        ).sum(axis=0)
        adv_grad_summand = 0
        negative_adv_grad_summand = (
...
def fair_adversarial_error_erm(
    X_test, w_gd, teacher_weights, epsilon, gamma, data_model, logger=None
):
    d = X_test.shape[1]
...
def compute_experimental_teacher_calibration(p, w, werm, Xtest, sigma):
    try:
        ...
        if sigma == 0:
            teacher_probabilities = np.array(
                [np.heaviside(Xtest[i] @ w / np.sqrt(d), 0.5) for i in index]
            )
        else:
            teacher_probabilities = np.array(
                [probit(Xtest[i] @ w / np.sqrt(d), sigma) for i in index]
            )
