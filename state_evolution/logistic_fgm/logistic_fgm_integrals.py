...
def alternative_derivative_f_out(
    ...
    second_derivative = numba_second_derivative_loss(
        y, proximal, epsilon * P / np.sqrt(N)
    )
...
def var_hat_func(task, overlaps, data_model) -> Overlaps:
...
def var_func(
    task, overlaps, data_model, slice_from=None, slice_to=None
) -> dict[str, float]:
    if slice_to is None:
    ...
    return {"m": m, "q": q, "sigma": sigma, "A": A, "P": P, "F": F, "N": N}
